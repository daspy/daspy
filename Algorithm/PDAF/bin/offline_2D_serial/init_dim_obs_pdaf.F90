...
SUBROUTINE init_dim_obs_pdaf(step, dim_obs_p)
    ...
    USE mod_assimilation, &
        ONLY : nx, ny, obs, obs_index, coords_obs, locweight, use_obs_mask, &
	...
	ALLOCATE(coords_obs(2, dim_obs_p))

    IF (ALLOCATED(R_Local)) DEALLOCATE(R_Local)
    IF (ALLOCATED(H_Local)) DEALLOCATE(H_Local)
    ALLOCATE(R_Local(dim_obs_p,dim_obs_p))
    ALLOCATE(H_Local(dim_obs_p,STATE_DIM))
    ...
    DO i = 1, STATE_DIM
        ...
        IF (OBS_NC(i) .ne. -9999.0) THEN
            ...
            IF (Normal_Score_Trans == 1) THEN
				...
				!CALL normal(ENSEMBLE_NUMBER,0.0,sqrt(R_NC(cnt,cnt)),Observation_Noise,OBS_NC(i))
